...
open class PoleZeroPair {
    @JvmField
    var poles: ComplexPair? = null

    @JvmField
    var zeros: ComplexPair? = null

    // single pole/zero
    constructor(p: Complex?, z: Complex?) {
        poles = p?.let { ComplexPair(it) }
        zeros = z?.let { ComplexPair(it) }
    }
    ...
    val isSinglePole: Boolean
        get() = (poles?.second?.equals(Complex(0.0, 0.0)) == true
                && zeros?.second?.equals(Complex(0.0, 0.0)) == true)

    fun is_nan(): Boolean {
        return poles?.is_nan() == true || zeros?.is_nan() == true
    }
}